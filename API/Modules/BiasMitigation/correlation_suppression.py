...
def correlation_suppression(
    ...
    """
    Apply correlation suppression and train/evaluate a model on the original and modified datasets.
    ...
    - correlation_threshold: float - Threshold for correlation above which features are removed.
    ...
    correlations = X_train.corrwith(X_train[sensitive_column])
    uncorrelated_features = correlations[
        correlations.abs() < correlation_threshold
    ].index.tolist()
    X_train_filtered = X_train[uncorrelated_features]
    X_test_filtered = X_test[uncorrelated_features]
    ...
    f1_score_filtered = f1_score(y_test, y_pred_filtered)

    metrics = {
        "predictions": y_pred_filtered.tolist(),
        "probabilities": probabilities.tolist(),
        "accuracy": float(accuracy_filtered),
        "precision": float(precision_filtered),
        "recall": float(recall_filtered),
        "f1_score": float(f1_score_filtered),
        "confusion_matrix": {
