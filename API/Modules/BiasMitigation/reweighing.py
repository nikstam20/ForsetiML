...
def relabel_data(
    ...
    """
    Apply Reweighing and train/evaluate a model on the original and modified datasets.
    ...
    reweighing = Reweighing(
        unprivileged_groups=[{sensitive_column: val} for val in unprivileged_classes],
        privileged_groups=[{sensitive_column: val} for val in privileged_classes],
    )
    train_ds_reweighted = reweighing.fit_transform(train_ds)

    X_train_reweighted = train_ds_reweighted.features
    y_train_reweighted = train_ds_reweighted.labels.ravel()
